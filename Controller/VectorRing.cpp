...
{
  if(i < maxSize){
    CopyVec(imuVec[i],outVec);
    return true;
  }
  return false;
}

void VectorRing::Peek(IMU& imuVec)
{
  CopyVec(imuVec[begin],outVec);
}
...
VectorRing::Push(IMU& imuVec)
{
  //copy first, then advance
  memcpy((void*)&imuRing[_begin],(void*)&imuVec,sizeof(IMU));
  advance();
  newData = true;
}
